...
describe('Date value => string', () => {
    it ('2020-08-01 => 2020-08-01', ()=>{
        expect(convertDateToFormattedString(rawDateOne)).toBe(expectResultDateString);
    })
    it ('2020/08/01 !=> 2020-08-01', ()=>{
        expect(convertDateToFormattedString(rawDateTwo)).not.toBe(expectResultDateString);
    })
});
...
describe('Date at N days from now', () => {
    it ('N = 7', ()=>{
        expect(getNDaysBeforeNow(numberOfDays).getDate()).toBe(expectResultDateValue.getDate());
    })
});
