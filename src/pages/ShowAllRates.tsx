...
const ShowAllRates = () => {
    ...
    //send actions
    const updateCurrency = (event: React.ChangeEvent<HTMLInputElement>) => dispatch(updateBaseCurrency(event.target.value));
    ...
    //send actions
    const updateDateValue = (event: React.ChangeEvent<HTMLInputElement>) => dispatch(updateDate(event.target.value));
            ...
            if (currenciesInShortName.includes(key as string)) {
                tableBody.push(
                    <StyledTableRow key={`${key}`}>
                        ...
                        <StyledTableCell
                            align="left">{getSymbolFromCurrency(key) + convertToThreeDecimals(value as number)}</StyledTableCell>
                    </StyledTableRow>
                )
            }
        }
